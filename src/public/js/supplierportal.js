...
(function ($) {
    ...
    (function (gb, $, undefined) {
		...
		gb.capitalize = function (user) {
        	...
        	var tmp = before.toLowerCase().replace( /(^| )(\w)/g, function(x){return x.toUpperCase();} ) + '';
        	return tmp;
		};
        ...
        gb.validateFreeEmail = function (emailAddress) {
            var pattern = new RegExp(/(aol|googlemail|msn|gmail|live|yahoo|outlook|hotmail)/);
    ...
    (function (gb, signup, $, undefined) {
        ...
        var signedAlready = false, aList = [], adminsList = [], suppliersList = [];
        ...
        var _s = {
        	...
        	isSupplierEmail : function (username) {
        		var result = true,
        			un = username.substring(username.indexOf("@") + 1);
        		
        		if ($.inArray(un, suppliersList) < 0) {
        			result = false;
        		}
        	...
        	isAdminEmail : function (username) {
        		var result = true,
        			un = username.substring(username.indexOf("@") + 1);
        		
        		if ($.inArray(un, adminsList) < 0) {
        			result = false;
        		}
        	...
        	showInvalidSupplierEmailMsg : function () {
        		$('#signupEmailValid').removeClass('smallStrong').addClass('smallWeek');
            	$('#signupEmailValid').text('This is not a pre-approved supplier email');
        	},
        	
        	showInvalidAdminEmailMsg : function () {
        		$('#signupEmailValid').removeClass('smallStrong').addClass('smallWeek');
            	$('#signupEmailValid').text('This is not a pre-approved admin email');
        	},
            	...
            	if ($('#signupUserName').val() !== '') {
                	if (_s.ValidEmail()) {
                    	if (_s.isApproved($('#signupUserName').val())) {
                    		if ($('#SignUpTitle').text().indexOf('Supplier') >= 0) {	
                    			if (_s.isSupplierEmail($('#signupUserName').val())) {
                    				_s.ShowValidEmailMsg();
                    			}
                    			else {
                    				_s.showInvalidSupplierEmailMsg();
                    			}
                    		}
                    		else {
                    			if (_s.isAdminEmail($('#signupUserName').val())) {
                    				_s.ShowValidEmailMsg();
                    			}
                    			else {
                    				_s.showInvalidAdminEmailMsg();
                    			}
                    		}
                    	}
                    	else {
        	...
        	getAdminsList : function () {
        		$.getJSON('k4e2550htyjrtyj4345435/adminslist', function(data) {
  					adminsList = [], suppliersList = [];
 
  					$.each(data, function(key, val) {
  						var vl = val.substring(val.indexOf("@") + 1);
  						
  						if ($.inArray(vl, adminsList) < 0) {
    						adminsList.push(vl);
    					}
    					
    					if ($.inArray(key, suppliersList) < 0) {
    						suppliersList.push(key);
    					}
  					});
				});
        	},
    ...
    (function (login, portal, $, undefined) {
    	...
    	var registeredDomain = '',
    		registeredDomain2 = '',
    		registeredViewerUrl = '';
    		registeredImagesReceiveFolder = '',
    		registeredValidations = '',
    		registeredDataDomain = ''; // Should be empty for Production - check domains.json
    	...
    	var currentPage = 0, imageUrl = '', activeApp = '', eFlowApps = [], collectionsRaw = [], collectionsGrid = [],
    		...
    		supplierId = '', supplierName = '', selected_PoNum = '', flip = false, canEditFields = false,
    		validationFailed = false, liveSync = false, oTable = null,
    		
    		supAdminsList = [], adminsList = [], suppliersList = [], fieldsToValidate = [], submittedByList = [];
    	...
    	var _p = {
    		...
    		getRegisteredDomains : function () {
        		$.getJSON('/k4e2550htyjrtyj4345435/domains', function(data) {
  					$.each(data, function(key, val) {
  						if (key === "registeredValidations") {
  							registeredValidations = val;
  						} else if (key === "registeredDomain") {
    						registeredDomain = val;
    					} else if (key === "registeredDomain2") {
    						registeredDomain2 = val;
    					} else if (key === "registeredViewerUrl") {
    						registeredViewerUrl = val;
    					} else if (key === "registeredImagesReceiveFolder") {
    						registeredImagesReceiveFolder = val;
    					} else if (key === "registeredDataDomain") {
    						registeredDataDomain = val;
    					}
  					});
				});
        	},
        	...
        	getAdminsList : function () {
        		$.getJSON('/k4e2550htyjrtyj4345435/adminslist', function(data) {
  					supAdminsList = [], adminsList = [], suppliersList = [];
  					...
  					$.each(data, function(key, val) {
    					...
    					if ($.inArray(supAdm, supAdminsList) < 0) {
    						supAdminsList.push(supAdm);
    					}
  					});
				});
        	},
    		...
    		initTable : function (aaData) {
    			...
    			oTable = $('#collections-list').dataTable({
    		...
    		getCurrentTablePage : function () {
    			var result = null;
    			
    			result = oTable.fnPagingInfo();
    			//console.log(result);
    			
    			if (result !== null || result !== undefined) {
    				if (result.iFilteredTotal === result.iTotal) { // no filter
    					if (result.iPage > 0) { // not on first page
    						return -1;
    					}
    				}
    				else { // filter
    		...
    		canShow : function (submittedBy) {
    			var result = false, 
    				sb = submittedBy.toLowerCase(),
    				dm = login.getDomain();
    			
    			if ($.inArray(dm, adminsList) >= 0) {
    				result = true;
    			}
    			else if ($.inArray(dm, suppliersList) >= 0) {
    				if (sb === dm.toLowerCase()) {
    					result = true;
    				}
    			}
    		...
    		getArrayDisplayField : function (cid, dt) {
    			...
    			if (dt.indexOf('|') >= 0) {
    				...
    				if (dtArr.length > 0) {
    					...
    					var submittedBy = _p.getSpec(dtArr, 'sb=');
    					
    					if (submittedBy === '') {
    						submittedBy = 'eFlow';
    					}
    					
    					if (_p.canShow(submittedBy)) {
    						...
    						if (dtArr[5] === '1') {
    							if (station.indexOf("REJECT") < 0 && station.indexOf("PENDING") < 0 && fileName !== '') {
    								...
    								} else if (station.toLowerCase() === "sportalafter") {
    									isIRApp = true;
    									station = 'Processed';
    									
    								} else if (station.toLowerCase() === "sportalbefore" || station.toLowerCase() === "sportalimport") {
    									isIRApp = true;
    									station = 'Progress';
    								}
    							}
    						...
    						if ($.inArray(bn, result) < 0) {
    							...
    							if (submittedBy.indexOf('.') > 0) {
    								submBy = submittedBy.substring(0, submittedBy.indexOf('.'));
    							}
    							else {
    								submBy = submittedBy;
    							}
    							
    							if (submBy === 'topimagesystems') {
    								submBy = 'tis';
    							} else if (submBy === 'doksend') {
    								submBy = 'dksend';
    							}
    							
    							submittedByList.push(bn + '|' + submittedBy);
    							
    							var nDate = date.replace('Monday', '').replace('Tuesday', '').replace('Wednesday', '').
    								replace('Thursday', '').replace('Friday', '').replace('Saturday', '').replace('Sunday', '');
    							
    							result.push(nDate, bn, station, status, submBy, vOrE, mn, un, ap, 
    							...
    							if (cl === cln) {
    								for (var j = 0; j <= dtArr.length - 1; j++) {
    									...
    									if (fld.indexOf('=') >= 0 && fld.indexOf('rc') < 0 
    										&& fld.indexOf('dc') < 0 && fld.indexOf('sb') < 0) {
    										fields.push(fld);
    									}
    								}
    							}
    		...
    		sendPageNum : function (page, burl) {
            	...
            	if (page !== undefined) {
            		var src = registeredViewerUrl + 'gview?url=' + burl + '&embedded=true';
    			...
    			if (__SupplierIdsPerUser.length === 0) {
    				...
    				for (var i = 0; i <= __SupplierIdsPerUser.length - 1; i++) {
    					...
    					if (p.strData !== undefined) {
    						...
    						if (item_user.toLowerCase() === user.toLowerCase()) {
    							found = true;
    						}
    		...
    		getSelectedSpId : function () {
    			return $('#uploadSuppliers').find(":selected").val();
    		},
    		
    		getSelectedSpName : function () {
    			return $('#uploadSuppliers').find(":selected").text();
    		},
    		
    		getSelectedPo : function () {
    			return $('#uploadPOs').find(":selected").val();
    		},
    		...
    		setDropUrl : function (sId, sN, pN) {
    			...
    			spId = _p.getSelectedSpId();
    			supplierId = spId;
    			
    			spName = _p.getSelectedSpName();
    			supplierName = spName;
    			
    			poNum = _p.getSelectedPo();
    			...
    			var ap = activeApp.substring(activeApp.indexOf('_') + 1),
    				...
    				viewerUrl = registeredDomain2 + 'dropfiles.php?app=' + ap + '&dm=' + login.getDomain() + 
    		...
    		supplierHasPo : function (supplier_id) {
    			var result = false;
    		...
    		sendSubmitUpload : function () {
                ...
                if (flip) {
                	...
                	_p.submitFlip(app, dm, spId, spName, poNum, user);
    			}
    		},
    		
    		submitFlip : function (app, dm, spId, spName, poNum, user) {
    			var urlVar = registeredDomain2 + 'upload.php?app=' + app + '&dm=' + dm + '&spId=' + spId + '&spName=' + spName + '&poNum=' + poNum + '&user=' + user;
    			...
    			$.ajax({
                    ...
                    success: function (data) {
                    	...
                    	$('#flipped').show().fadeOut(1550, function() {
    		...
    		hideDropArea : function () {
    			$('#showDropArea').text("Upload Image(s)");
    			$('#dropzone1').hide();
    			$('#dropzone2').hide();
    			$('#dropzone').hide();
    			$('#batchzone').show();
    		...
    		showFlipArea : function () {
    			$('#uploadtag').text('Flip PO for ' + activeApp.substring(activeApp.indexOf('_') + 1));
    			$('#uploadSubmitBtnEnabled').text('Flip');
    			$('#dropzone').show();
    			$('#dropzone1').show();
    			$('#dropzone2').hide();
    			$('#batchzone').hide();
    		...
    		showDropArea : function () {
    			$('#uploadtag').text('Upload images for ' + activeApp.substring(activeApp.indexOf('_') + 1));
    			$('#uploadSubmitBtnEnabled').text('Upload');
    			$('#dropzone').show();
    			$('#dropzone1').show();
    			$('#dropzone2').hide();
    			$('#batchzone').hide();
    		...
    		runAllValidations : function () {
    			for (var i = 0; i <= fieldsToValidate.length - 1; i++) {
    				var fn = fieldsToValidate[i], 
    					fId = '#' + _p.fieldToId(fn),
    				
    					fieldId = $(fId).attr('id'),
						fieldValue = $(fId).val(),
						app = activeApp.substring(activeApp.indexOf('_') + 1),
						dm = activeApp.substring(0, activeApp.indexOf('_'));
				
					_p.runFieldValidation(fieldId, fieldValue, app, dm, canEditFields, false);
    			}
    		},
    		
    		getCollectionName : function () {
    			return $('#viewerCollection').text();
    		},
    		
    		getFieldStr : function (atvApp) {
    			var result = 'c_' + _p.getCollectionName() + '|' + atvApp + '|';
    			...
    			for (var i = 0; i <= fieldsToValidate.length - 1; i++) {
					...
					fieldId = fieldId.replace('__', '$');
					
					if (i < fieldsToValidate.length - 1) {
						pipe = fieldId + '=' + fieldValue + '|';
					} else {
						pipe = fieldId + '=' + fieldValue;
					}
    		...
    		getSupAdmin : function (user) {
    			var result = '';
    			
    			for (var i = 0; i <= supAdminsList.length - 1; i++) {
    				var item = supAdminsList[i];
    				
    				if (item.toLowerCase().indexOf(user.toLowerCase()) >= 0) {
    					var itms = item.split('=');
    		...
    		getUserTables : function () {
    			var result = [], cn = _p.getCollectionName(), 
    				dm = activeApp.substring(0, activeApp.indexOf('_'));
    				
    			if ($.inArray(dm, suppliersList) >= 0) {	
    				var supAdmin = _p.getSupAdmin(dm);
    				
    				if (supAdmin !== '') {
    					result.push(supAdmin);
    				}
    			}
    			
    			if (submittedByList.length > 0) {
    				for (var i = 0; i <= submittedByList.length - 1; i++) {
    					var sbyItem = submittedByList[i],
    						item = sbyItem.split('|'),
    						collec = item[0],
    						sb = item[1];
    					
    					if (collec === cn) {
    						if (dm.toLowerCase() !== sb.toLowerCase()) {
    							result.push(sb);
    						}
    					}
    				}
    		...
    		submitOrNot : function () {
    			
    			timer = $.timer(function() {
    				if (validationFailed) { // Validation failed
    					timer.stop();
    					
    					$.createNotification({
							horizontal: 'right',
							vertical: 'bottom',
							content: '<strong>Batch NOT submitted</strong><br /><br />Some fields failed validation<br /><br />Please check',
							duration: 2500
						});
    				}
    				else { // Save batch
    					...
    					var usertables = _p.getUserTables();
    					
    					var fieldStr = _p.getFieldStr(activeApp),
    						sbUrl = '/k4e2550htyjrtyj4345435/radkiddo/eafa/sp/efl/timgsys/api/cd/submitbatch';
    					
    					$.ajax({
                			url: sbUrl,
                    		type: 'POST',
                    		cache: false,
                    		data: 'details=' + fieldStr + '&replace=true',
                    		headers: { "cache-control": "no-cache" },
                    		success: function (data) {
								...
								$.createNotification({
									horizontal: 'right',
									vertical: 'bottom',
									content: '<strong>Success</strong><br /><br />Hooray!<br /><br />Batch submitted',
									duration: 2500
								});
								
								var dm = activeApp.substring(0, activeApp.indexOf('_')),
								app = activeApp.substring(activeApp.indexOf('_') + 1);
								
								_p.updateOtherTables(fieldStr, activeApp, app, dm, sbUrl, usertables);
                    		},
                    		error: function (jqXHR, textStatus, err) {}
                		});
    				}
    		...
    		updateOtherTables : function (fieldStr, atvApp, app, dm, sbUrl, usertables) {
    			if (usertables.length > 0 ) {
    				for (var i = 0; i <= usertables.length - 1; i++) {
    					var dmName = usertables[i],
    						apName = dmName + '_' + app;
    					
    					fieldStr = fieldStr.replace(atvApp, apName);
    		...
    		submitBatch : function () {
    			...
    			_p.runAllValidations();
    			_p.submitOrNot();
    		},
    		
    		splitFieldInTwoLines : function (str) {
    			var result = '';
    			
    			if (str.indexOf('__') >= 0) {
    				result = str.substring(0, str.indexOf('__')) + 
    					'<br />' + str.substring(str.indexOf('__'));
    			}
    			else
    				result = str;
    		...
    		runFieldValidation : function (fieldId, fieldValue, app, domain, canEditFlds, showMsg) {
    			
    			if (canEditFlds) {
    				var server = registeredValidations + 'validations.php';
    						...
    						if (message !== '') {
    							...
    							if (showMsg) {
    								$.createNotification({
										horizontal: 'right',
										vertical: 'bottom',
										content: '<strong>[' + _p.splitFieldInTwoLines(fieldId) + ']</strong><br /><br />Not correct: <br /><br />' + message,
										duration: 2500
									});
								}
                            }
					...
					socket.postMessage(fieldId + '|' + fieldValue + '|' + app + '|' + domain);
				}
    		},
    		
    		activateOrNotLiveSync : function () {
    			if (_p.getCurrentTablePage() === 0) {
    				liveSync = true;
    				//console.log('sync on');
    			}
    			else {
    	...
    	portal.initTable = function () {
    		liveSync = true;
    		activeApp = '';
    		_p.getRegisteredDomains();
    		_p.getAdminsList();
    	
    		var loginTimer = $.timer(function() {
    			if (!_p.isLoggedIn() && window.location.href.indexOf('#/portal') >= 0) {
    				window.location.href = '#/';
    			}
    		});
    		
    		loginTimer.set({ time : 1500, autostart : true });
    	...
    	$('#liveSync').click(function (e) {
    		e.preventDefault();
    		
    		if ($('#liveSync').text() === 'Live Sync: ON') {
    			liveSync = false;
    			$('#liveSync').text('Live Sync: OFF');
    		}
    		else {
    			liveSync = true;
    			$('#liveSync').text('Live Sync: ON');
    		}
    	...
    	$('#showDropArea').click(function (e) {
    		...
    		if ($('#showDropArea').text() === "Upload Image(s)") {
    			_p.showDropArea();
		...
		$('#fieldsListViewer1').on('keyup', 'input', function(e)
		{
			if ( e.keyCode === 13 ) {
				...
				// Just for debugging... remove the line below for production
				//canEditFields = true;
				
				validationFailed = false;
				_p.runFieldValidation(fieldId, fieldValue, app, dm, canEditFields, true);	
			}
		});
    	...
    	$('#colleclist').delegate('a', 'click', function(e) {
    		...
    		var cln = $(this).attr('href'), 
    			viewOrEdit = $(this).text();
    			
    		if (viewOrEdit === 'View') {
    			canEditFields = false;
    			$('#submitBatch').text('');
    		}
    		else if (viewOrEdit === 'Edit' || viewOrEdit === 'ReEdit') {
    			canEditFields = true;
    			validationFailed = false;
    			$('#submitBatch').text('Save Batch');
    		}
    ...
    (function (adminsignup, portal, viewer, usersettings, recover, login, signup, routing, $, undefined) {
        ...
        var _r = {
        	...
        	hideAll : function () {
            	...
            	$('#approvalFailedDiv').hide();
            	$('#alreadyActivatedDiv').hide();
            	$('#alreadyApprovedDiv').hide();
            	$('#activationSuccessfulDiv').hide();
            	$('#approvalSuccessfulDiv').hide();
        	...
        	approvalFailedPage : function () {
            	_r.menuMain();
            	_r.hideAll();
            	$('#approvalFailedDiv').show();
        	},
        	...
        	approvalSuccessful : function () {
        		_r.menuMain();
            	_r.hideAll();
            	$('#approvalSuccessfulDiv').show();
        	},
        	...
        	route2Page : function (rt) {
            	if (rt === '' && window.location.href.indexOf('#/') < 0) {
            	...
            	} else if (rt === 'approvalfailed') {
                	_r.approvalFailedPage();
            	} else if (rt === 'activationusernotfound') {
                	_r.alreadyActivated();
            	} else if (rt === 'approvalusernotfound') {
                	_r.alreadyApproved();
                	...
                	_r.usersettingsPage();
            	} else if (rt === 'approvalsuccessful') {
            		_r.approvalSuccessful();
            	}
        	}
        };
        ...
        var routes = {
            ...
            '/approvalfailed': _r.approvalFailedPage,
            '/activationusernotfound': _r.alreadyActivated,
            '/approvalusernotfound': _r.alreadyApproved,
            '/activationsuccessful': _r.activationSuccessful,
            '/approvalsuccessful': _r.approvalSuccessful,
