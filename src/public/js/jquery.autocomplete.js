    ...
    Autocomplete.formatResult = function (suggestion, currentValue) {
        ...
        if (currentValue.indexOf(' ') >= 0) {
        	...
        	for (var i = 0; i <= arr.length - 1; i++) {
        		...
        		if (suggestion.value.toLowerCase().indexOf(str.toLowerCase()) >= 0) { 
        			pattern = '(' + str.replace(reEscape, '\\$1') + ')';
        			
        			strTemp = suggestion.value.replace(new RegExp(pattern, 'gi'), '<strong>$1</strong>');
        		}
        	}
        	
        	//suggestion.value = strTemp;
    ...
    Autocomplete.prototype = {
        ...
        getSuggestionsLocal: function (query) {
            var that = this,
                ...
                itemsRaw = that.options.lookup,
                arrStr = queryLowerCase.split(' '),
                itemsSearched = [], itemS = [];
    			...
    			if (arrStr !== undefined && arrStr.length > 0) {
    				...
    				for (var i = 0; i < arrStr.length; i++) {
    					if (arrStr[i] !== '' && arrStr[i] !== ' ') {
    						var value = arrStr[i].toString();
    						
    						for (var j = 0; j <= itemsRaw.length - 1; j++) {
    							var itm = itemsRaw[j].value.toString();
    						
    							if (itm.toLowerCase().indexOf(value.toLowerCase()) >= 0) {
    								var dta = itm,
    									val = itm;
    								
    								if ($.inArray(dta, itemS) < 0) {
    									itemS.push(dta);
    									itemsSearched.push( { data : dta, value : val } );
    								}
    							}
    						}
            ...
            return { 
            	suggestions: itemsSearched
                /*suggestions: $.grep(itemsSearched, function (suggestion) {
        ...
        suggest: function () {
            ...
            $.each(that.suggestions, function (i, suggestion) {
            	var res = formatResult(suggestion, value);

                html += '<div class="' + className + '" data-index="' + i + '">' + res + '</div>';
            });
